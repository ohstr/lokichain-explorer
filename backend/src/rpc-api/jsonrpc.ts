...
JsonRPC.prototype.call = function (method, params) {
  return new Promise((resolve, reject) => {
    ...
    requestOptions.ca = readFileSync(this.opts.sslCa).toString();
  }
    ...
    request.on('response', (response) => {
      ...
      response.on('end', () => {
        ...
        try {
          ...
          // console.log("responseDecoded", decoded)
          ...
          if (response.statusCode !== 200) {
            ...
            err = new Error(`Problem parsing JSON response from server buffer: ${buffer} req: ${requestJSON}`)
            err.code = -32603
            reject(err)
          }
          return
