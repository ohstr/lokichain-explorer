...
export class BlockFeeRatesGraphComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.widget) {
      ...
      this.seoService.setDescription($localize`:@@meta.description.bitcoin.graphs.block-fee-rates:See Flokicoin feerates visualized over time, including minimum and maximum feerates per block along with feerates at various percentiles.`);
      ...
      tooltip: {
        ...
        formatter: function(data) {
          ...
          for (const rate of data.reverse()) {
            if (weightMode) {
              tooltip += `${rate.marker} ${rate.seriesName}: ${(rate.data[1] / 4).toFixed(2)} loki/WU<br>`;
            } else {
              tooltip += `${rate.marker} ${rate.seriesName}: ${rate.data[1].toFixed(2)} loki/vByte<br>`;
            }
          }
