...
export class BlockFeesSubsidyGraphComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.seoService.setDescription($localize`:@@meta.description.bitcoin.graphs.block-fees-subsidy:See the mining fees earned per Flokicoin block compared to the Flokicoin block subsidy, visualized in FLC and USD over time.`);
      ...
      tooltip: {
        ...
        formatter: function (data) {
          ...
          for (let i = data.length - 1; i >= 0; i--) {
            ...
            if (this.displayMode === 'normal') tooltip += `${formatNumber(tick.data, this.locale, '1.0-3')} FLC<br>`;
          ...
          if (this.displayMode === 'normal') tooltip += `<div style="margin-left: 2px">${formatNumber(data.reduce((acc, val) => acc + val.data, 0), this.locale, '1.0-3')} FLC</div>`;
      ...
      yAxis: this.data.blockFees.length === 0 ? undefined : [
        {
          ...
          axisLabel: {
            color: 'var(--grey)',
            formatter: (val) => {
              return `${val}${this.displayMode === 'percentage' ? '%' : ' FLC'}`;
            }
          },
          min: 0,
          max: (value) => {
