...
export class BlockRewardsGraphComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.seoService.setDescription($localize`:@@meta.description.bitcoin.graphs.block-rewards:See Flokicoin block rewards in FLC and USD visualized over time. Block rewards are the total funds miners earn from the block subsidy and fees.`);
        ...
        formatter: function (data) {
          ...
          for (const tick of data) {
            if (tick.seriesIndex === 0) {
              tooltip += `${tick.marker} ${tick.seriesName}: ${formatNumber(tick.data[1], this.locale, '1.3-3')} FLC<br>`;
