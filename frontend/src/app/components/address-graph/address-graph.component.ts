...
export class AddressGraphComponent implements OnChanges, OnDestroy {
  ...
  prepareChartOptions(summary: AddressTxSummary[]) {
    ...
    if (this.period !== 'all') {
      ...
      const startFiat = this.data[0]?.[0] ?? start; // Make sure USD data starts at the same time as FLC data
        ...
        formatter: function (data) {
          ...
          if (btcData.length && fiatData.length) {
            tooltip += `<div style="display: flex; justify-content: space-between; color: ${btcColor}">
              <span style="text-align: left; margin-right: 10px;">${btcSymbol} ${formatBTC(btcVal, 4)} FLC</span>
              <span style="text-align: right;">${fiatSymbol} ${formatFiat(fiatVal)}</span>
            </div>
            <div style="display: flex; justify-content: space-between;">
              <span style="text-align: left; margin-right: 10px;">${formatBTC(btcData[0].data[1], 4)} FLC</span>
              <span style="text-align: right;">${formatFiat(fiatData[0].data[1])}</span>
            </div>`;
          } else if (btcData.length) {
            tooltip += `<span style="color: ${btcColor}">${btcSymbol} ${formatBTC(btcVal, 8)} FLC</span><br>
              <span>${formatBTC(data[0].data[1], 8)} FLC</span>`;
          } else {
            if (this.selected[$localize`:@@7e69426bd97a606d8ae6026762858e6e7c86a1fd:Balance`]) {
              tooltip += `<div style="display: flex; justify-content: space-between;">
                <span style="text-align: left; margin-right: 10px;">${formatBTC(data[0].data[3], 4)} FLC</span>
          ...
          axisLabel: {
            ...
            formatter: (val): string => {
              ...
              if (valSpan > 100_000_000_000) {
                return `${this.amountShortenerPipe.transform(Math.round(val / 100_000_000), 0, undefined, true)} FLC`;
              }
              else if (valSpan > 1_000_000_000) {
                return `${this.amountShortenerPipe.transform(Math.round(val / 100_000_000), 2, undefined, true)} FLC`;
              } else if (valSpan > 100_000_000) {
                return `${(val / 100_000_000).toFixed(1)} FLC`;
              } else if (valSpan > 10_000_000) {
                return `${(val / 100_000_000).toFixed(2)} FLC`;
              } else if (valSpan > 1_000_000) {
                if (maxValue > 100_000_000_000) {
                  return `${this.amountShortenerPipe.transform(Math.round(val / 100_000_000), 3, undefined, true)} FLC`;
                }
                return `${(val / 100_000_000).toFixed(3)} FLC`;
              } else {
                return `${this.amountShortenerPipe.transform(val, 0, undefined, true)} loki`;
              }
            }
          },
          splitLine: {
