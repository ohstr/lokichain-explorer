...
export class NodesPerISPChartComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (!this.widget) {
      ...
      this.seoService.setDescription($localize`:@@meta.description.lightning.nodes-per-isp:Browse the top 100 ISPs hosting Lightning nodes along with stats like total number of nodes per ISP, aggregate FLC capacity per ISP, and more`);
    }
        ...
        tooltip: {
          ...
          formatter: () => {
            ...
            return `<b style="color: white">${isp[1]} (${this.sortBy === 'capacity' ? isp[7] : isp[6]}%)</b><br>` +
              ...
              $localize`${this.amountShortenerPipe.transform(isp[2] / 100000000, 2)} FLC`
            ;
          }
        },
        data: isp[0],
      ...
      tooltip: {
        ...
        formatter: () => {
          ...
          return `<b style="color: white">` + $localize`Other (${totalShareOther.toFixed(2) + '%'})` + `</b><br>` +
            $localize`${nodeCount} nodes` + `<br>` +
            $localize`${this.amountShortenerPipe.transform(capacityOther / 100000000, 2)} FLC`;
        }
      },
      data: 9999 as any,
    } as PieSeriesOption);
