    ...
    this.nodes$ = this.route.paramMap
      .pipe(
        ...
        map(response => {
          ...
          this.seoService.setDescription($localize`:@@meta.description.lightning.nodes-isp:Browse all Flokicoin Lightning nodes using the ${response.isp} [AS${this.route.snapshot.params.isp}] ISP and see aggregate stats like total number of nodes, total capacity, and more for the ISP.`);
