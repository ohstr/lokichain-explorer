...
export class LightningStatisticsChartComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    if (this.widget) {
      ...
      this.seoService.setDescription($localize`:@@meta.description.lightning.stats-chart:See the capacity of the Lightning network visualized over time in terms of the number of open channels and total flokicoin capacity.`);
        ...
        formatter: (ticks): string => {
          ...
          for (const tick of ticks) {
            if (tick.seriesIndex === 0) { // Channels
              ...
              weightString = `${tick.marker} ${tick.seriesName}: ${formatNumber(tick.data[1] / 100000000, this.locale, '1.0-0')} FLC`;
            }
          }
