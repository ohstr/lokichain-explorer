...
export class NodesPerCountryChartComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.seoService.setDescription($localize`:@@meta.description.lightning.nodes-country-overview:See a geographical breakdown of the Lightning network: how many Lightning nodes are hosted in countries around the world, aggregate FLC capacity for each country, and more.`);
        ...
        tooltip: {
          ...
          formatter: () => {
            ...
            return `<b style="color: white">${country.name.en} (${country.share}%)</b><br>` +
              ...
              $localize`${this.amountShortenerPipe.transform(country.capacity / 100000000, 2)} FLC capacity`
            ;
          }
        },
        data: country.iso,
